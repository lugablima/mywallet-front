...
export default function SignUpPage() {
  const [inputs, setInputs] = useState({ name: "", email: "", password: "", repeat_password: "" });
  ...
  async function handleForm(e) {
    ...
    if (!isValidName(inputs.name)) return alert("Nome inválido, tente novamente!");

    if (!isValidEmail(inputs.email)) return alert("E-mail inválido, tente novamente!");

    if (inputs.password !== inputs.repeat_password) return alert("Repita a senha corretamente!");
    ...
    <Container>
      ...
      <form onSubmit={handleForm}>
        ...
        <input
          ...
          onChange={(e) => setInputs({ ...inputs, repeat_password: e.target.value })}
        />
